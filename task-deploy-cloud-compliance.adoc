...
また、link:task-deploy-compliance-onprem.html["インターネットにアクセスできる Linux ホストにデータ分類をインストールする"]こともできます。オンプレミスに配置された NetApp Data Classification インスタンスを使用してオンプレミス ONTAP システムをスキャンする場合は、このタイプのインストールが適切な選択肢となる場合があります。ただし、これは必須ではありません。どのインストール方法を選択しても、ソフトウェアの動作はまったく同じです。
...
コンソールエージェントがまだない場合は、クラウド プロバイダでコンソールエージェントを作成してください。参照： https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-aws.html["AWSでコンソールエージェントを作成する"^]または https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-azure.html["Azureでコンソールエージェントを作成する"^]、または https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-google.html["GCP でコンソール エージェントを作成する"^]。ほとんどの場合、Data Classification をアクティブ化する前にコンソールエージェントがセットアップされているはずです（ほとんどの https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html["コンソール機能にはコンソールエージェントが必要です"]）が、今すぐ設定する必要がある場合もあります。

特定のクラウド プロバイダにデプロイされたコンソールエージェントを使用する必要があるシナリオがいくつかあります。
