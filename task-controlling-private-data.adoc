...
---
= NetApp Data Classificationを使用して組織の個人データに関するコンプライアンスの詳細を表示
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: ファイルレベルのコンプライアンス詳細は、完全な分類スキャンを実行した場合にのみ確認できます。マップのみのスキャンでは、ファイルレベルの詳細情報は得られません。
...
+
image:screenshot_compliance_investigation_page.png["「調査結果」を選択した後にファイル内で見つかった詳細情報のスクリーンショット。"]

+
image:screenshot_compliance_investigation_page_directory.png["「調査結果」を選択した後にディレクトリで見つかる詳細情報のスクリーンショット。"]
...
. 特定のファイルについて、検索、並べ替え、詳細の展開を行う、*結果の調査*を選択してマスクされた情報を表示する、またはファイルリストをダウンロードするなどして、データを調査します。
