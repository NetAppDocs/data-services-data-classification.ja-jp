...
詳細については、link:https://docs.netapp.com/us-en/data-services-data-classification/task-health-monitor.html["データ分類の健全性を監視する"^]を参照してください。
...
このBlueXP分類リリースには、リソース利用を最適化するコード変更が含まれています。
...
BlueXP classificationは作業環境の一括操作をサポートします。作業環境において、マッピング スキャンを有効にするか、マップと分類スキャンを有効にするか、スキャンを無効にするか、ボリューム全体にカスタム構成を作成するかを選択できます。個々のボリュームを選択した場合は、一括選択が上書きされます。一括操作を実行するには、**構成**ページに移動して選択を行います。
...
マップのみのスキャンを実行する場合、最も重要なスキャンを優先することができます。この機能は、複数の作業環境があり、優先度の高いスキャンを優先的に完了させたい場合に役立ちます。
...
優先順位はlink:https://docs.netapp.com/us-en/data-services-data-classification/concept-classification.html["マップのみのスキャン"^]に限定されます。マップおよび分類スキャンには使用できません。
...
今回のリリースには、軽微な機能強化が含まれています。

このリリースでは、次のオプションも削除されました。

* 監査ログ収集を有効にするオプションが無効になっています。
* ディレクトリ調査中は、ディレクトリごとに個人識別情報（PII）データの数を計算するオプションは利用できません。link:task-investigate-data.html["組織内に保存されているデータを調査する"]を参照してください。
...
Red Hat Enterprise Linux バージョン 8 および 9 は、以前 BlueXP 分類のインストールに必要だった Docker エンジンをサポートしていません。BlueXP 分類は、コンテナインフラストラクチャとして Podman バージョン 4 以降を使用することで、RHEL 9.0、9.1、および 9.2 へのインストールをサポートするようになりました。お使いの環境で最新バージョンの RHEL を使用する必要がある場合は、Podman を使用して BlueXP 分類（バージョン 1.26 以降）をインストールできます。

現在、RHEL 9.x を使用する場合、Data Classification はダークサイトへのインストールや分散スキャン環境（マスターノードとリモートスキャナノードを使用）をサポートしていません。
...
「アクセス権を持つユーザー数」フィルターは、SharePoint Onlineリポジトリに保存されているファイルをサポートするようになりました。以前は、CIFS共有上のファイルのみがサポートされていました。SharePointグループのうち、Active Directoryベースではないものは、現時点ではこのフィルターの対象外です。
