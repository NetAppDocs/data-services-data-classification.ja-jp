...
[role="lead"]
NetApp Data Classification REST API を使用して、Web UI で利用できる機能と同じ機能にアクセスできます。
...
** データは JSON 形式でエクスポートされます（Splunk などのサードパーティ アプリケーションへの解析や送信が容易です）


* 「AND」および「OR」ステートメントを使用してクエリを作成し、情報を含めたり除外したりします。
+
例えば、特定の個人識別情報（PII）を含まないファイルを検索できます（この機能はUIでは利用できません）。エクスポート操作から特定のフィールドを除外することもできます。
...
Swagger にアクセスするには、NetApp Data Classification インスタンスの IP アドレスが必要です。クラウド環境への展開には、パブリック IP アドレスを使用してください。次に、このエンドポイントにアクセスする必要があります：
...
.API 要求
調査タブですべてのフィルターを表示するには、システムの関連フィールドとオプションをすべて取得する必要があります。
...
----
リクエストパラメータのレスポンスを使用して、コピーするファイルをフィルタリングします。
