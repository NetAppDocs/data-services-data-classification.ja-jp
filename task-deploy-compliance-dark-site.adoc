...
[role="lead"]
インターネットアクセスがないオンプレミスサイトの Linux ホストに NetApp Data Classification をインストールするには、_プライベートモード_を使用します。このデプロイではインストールスクリプトを使用し、Data Classification インスタンスを NetApp Console SaaS レイヤーから切断された状態に維持するため、隔離された環境でローカルデータソースをスキャンできます。
