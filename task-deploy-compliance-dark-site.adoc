...
---
= プライベートモードのLinuxホストに NetApp Data Classification をインストールする
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
インターネットにアクセスできないオンプレミスサイトのLinuxホストに、オフラインインストール（ダークサイトとも呼ばれます）でNetApp Data Classificationをインストールするには、_プライベートモード_を使用します。このデプロイではインストールスクリプトを使用し、Data ClassificationインスタンスをNetApp Console SaaSレイヤから切断された状態に維持するため、隔離された環境でローカルデータソースをスキャンできます。
...
データ分類のインストールスクリプトは、システムと環境が必要な前提条件を満たしているかどうかを確認します。前提条件を満たしている場合、インストールが開始されます。データ分類のインストールとは別に前提条件を確認したい場合は、前提条件のみをテストする別のソフトウェアパッケージをダウンロードできます。link:task-test-linux-system.html["Linuxホストがデータ分類をインストールする準備ができているかどうかを確認する方法をご覧ください"]。
...
プライベートモードでインストールした場合、NetApp Data Classificationは、オンプレミスサイトにもローカルに存在するデータソースのデータのみをスキャンできます。現時点では、NetApp Data Classificationは次の*ローカル*データソースをスキャンできます：
...
NetApp Data Classification がコンソールエージェントにデプロイされている場合、コンソールエージェントを別のホストに移行することはサポートされていません。ダークサイトで別のコンソールエージェントに NetApp Data Classification が必要な場合は、新しいホストに新しいコンソールエージェントと NetApp Data Classification インスタンスをデプロイしてください。
...
* NetApp Data Classification は専用ホスト上で実行する必要があります。ホストは、他のアプリケーションや、ウイルス対策ソフトなどのサードパーティ製ソフトウェアと共有することはできません。
...
** *Azure VM サイズ*："Standard_D16_v5"。link:reference-instance-types.html#azure-instance-types["その他のAzureインスタンスタイプを見る"^]。
...
* *Red Hat Subscription Management*：ホストは Red Hat Subscription Management に登録されている必要があります。登録されていない場合、システムはインストール中に必要なサードパーティ製ソフトウェアを更新するためのリポジトリにアクセスできません。
...
NOTE: データ分類インストーラーは、UID 4321とGID 4321を予約します。インストールを開始する前に、ホストマシン上でUID 4321とGID 4321が使用されていないことを確認してください。それらが既に使用されている場合、インストールは「invalid user」というエラーメッセージとともに失敗します。
...
ほとんどの場合、Data Classification は単一ホストシステムにインストールします。これはコンソールエージェントと同じ VM ホストではありません。
